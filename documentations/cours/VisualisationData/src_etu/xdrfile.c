...
/* 
 * For portable binary files we use the RPC/XDR standard, which
 * is a core part of NFS and TCP/IP on any unix system.
 * (i.e. we represent numbers in the network standard format).
 * This is much more portable than just checking for big/small endian;
 * even non-IEEE machines (e.g. VAX) support RPC/XDR, and there
 * are some architecture that use big-endian within a word, but small
 * endian word order for double-precision floating point (yuck.)
 *
 * The safest solution is to use the RPC/XDR library from your vendor.
 * Since it is required for NFS it should be present on any Unix OS.
 * To do this, define HAVE_XDR when compiling. You might have to link
 * with -lnsl or some other library.
 *
 * By default we use our own implementation of the XDR routines.
 * This is useful on MS Windows or if you cannot find the libraries.
 */

#ifdef HAVE_XDR
#include <rpc/rpc.h>
#include <rpc/xdr.h>
#else /* Compiling without system RPC/XDR libraries */

/*
* Define our own subset of XDR routines statically.
 * The actual implementation is done at the end of this file
 * if necessary (if you dont use the system libraries).
 */
enum xdr_op {
  XDR_ENCODE = 0,
  XDR_DECODE = 1,
 ...
 /* define integer types that are guaranteed to be 4 bytes wide */
...
#  error Use the system XDR libraries, or edit xdrfile.c
#endif
...
struct XDR {
  ...
  struct xdr_ops
  {
    int (*x_getlong) (XDR *__xdrs, xdr_int32_t *__lp);
    int (*x_putlong) (XDR *__xdrs, xdr_int32_t *__lp);
...
off_t
xdrfile_getpos(XDRFILE *xfp)
{
  return xdr_getpos((XDR *)xfp->xdr);
}

int
xdrfile_setpos(XDRFILE *xfp, off_t pos)
{
  return xdr_setpos((XDR *)xfp->xdr,pos);
}
...
//int
void
xdrfile_setpos(XDRFILE *xfp, long pos) //unsigned int pos)
{
    fseek(xfp->fp,pos,SEEK_SET);
//  return xdr_setpos((XDR *)xfp->xdr,pos);
}
...
static void encodeints(int buf[], const int num_of_ints, const int num_of_bits,
...
static void decodeints(int buf[], const int num_of_ints, int num_of_bits,
 ...
 */
int
xdrfile_read_compr_coord_float(float     *fp, /* length 3*ncoord */ 
			       int       *size,
			       float     *precision,
			       XDRFILE*   xfp)
{
  ...
  if(tmp==0)
    return 0; /* return if we could not read size */
  if (*size != 0 && lsize != *size) {
    fprintf(stderr, "Requested to read %d compressed coords, file contains %d\n",
	    *size, lsize);
    return 0;
  }
  *size = lsize;
  ...
  if(size3>xfp->buf1size) {
    if((xfp->buf1=(int *)malloc(sizeof(int)*size3))==NULL) {
      fprintf(stderr,"Cannot allocate memory for reading compressed coordinates.\n");
      return 0; 
    }
    xfp->buf1size=size3;
  ...
  if(*size<=9) {
    return xdrfile_read_float(fp,size3,xfp)/3;
  ...
  while ( i < lsize ) {
	...
	if (k == 0) {
	  ...
	  prevcoord[2] = tmp;
	  *lfp++ = prevcoord[0] * inv_precision *10.0;
	  *lfp++ = prevcoord[1] * inv_precision *10.0;
	  *lfp++ = prevcoord[2] * inv_precision *10.0;
	} else {
	  prevcoord[0] = thiscoord[0];
	  prevcoord[1] = thiscoord[1];
	  prevcoord[2] = thiscoord[2];
	}
	*lfp++ = thiscoord[0] * inv_precision *10.0;
	*lfp++ = thiscoord[1] * inv_precision *10.0;
	*lfp++ = thiscoord[2] * inv_precision *10.0;
      }
    } else {
      *lfp++ = thiscoord[0] * inv_precision *10.0;
      *lfp++ = thiscoord[1] * inv_precision *10.0;
...
int
xdrfile_write_compr_coord_float(float   *fp, /* length 3*ncoord */ 
				int      size,
				float    precision,
				XDRFILE* xfp)
{
  ...
  if(xdrfile_write_int(&size,1,xfp)==0)
    return 0; /* return if we could not write size */
  /* Dont bother with compression for three atoms or less */
  if(size<=9) {
    return xdrfile_write_float(fp,size3,xfp)/3;
    /* return number of coords, not floats */
  }
  ...
  while(lfp < fp + size3 ) {
    ...
    if (fabs(lf) > INT_MAX-2) {
      ...
      fprintf(stderr,"Internal overflow writing compressed coordinates.\n");
      errval=0;
    }
    lint1 = lf;
...
int
xdrfile_read_compr_coord_double(double     *fp, /* length 3*ncoord */ 
				int        *size,
				double     *precision,
				XDRFILE*   xfp)
{
...
int
xdrfile_write_compr_coord_double(double   *fp, /* length 3*ncoord */ 
				 int      size,
				 double    precision,
				 XDRFILE* xfp)
{
 ...
 * (as they are when called from C) since we need a static   *
 * array to keep track of file pointers.                     *
 * In the _extremely_ unlikely event that you are doing      *
 * multithreaded IO in Fortran you could just add a mutex,   *
 * but I dont do it by default to keep this file simple.     *
 *************************************************************/
#define MAX_FORTRAN_XDR 1024
static XDRFILE *f77xdr[MAX_FORTRAN_XDR]; /* only used in Fortran */
static int      nf77xdr = -1;            /* only used in Fortran */
...
void
F77_FUNC(xdropen,XDROPEN)(int *fid, char *filename, char *mode, int *ret,
			  int fn_len, int mode_len)
{
  ...
  i=0;
  while(i<nf77xdr && f77xdr[i]!=NULL)
    i++;
  if(i==MAX_FORTRAN_XDR) {
    *fid = *ret = -1;
    return;
  } 
  if (ftocstr(cfilename, sizeof(cfilename), filename, fn_len)) {
    *fid = *ret = -1;
    return;
  }
  if (ftocstr(cmode, sizeof(cmode), mode,mode_len)) {
    *fid = *ret = -1;
    return;
  }
  f77xdr[i]=xdrfile_open(cfilename,cmode);
  *fid=i; /* use the index in the array as a fortran file handle */
  *ret=0;
}

void
F77_FUNC(xdrclose,XDRCLOSE)(int *fid, int *ret)
{
  *ret=xdrfile_close(f77xdr[*fid]);
}

/* Dont try 64-bit fseek in fortran, but read/write should be ok... */
void
F77_FUNC(xdrgetpos,XDRGETPOS)(int *fid, int *pos, int *ret)
{
  if(*fid>=0 && *fid<MAX_FORTRAN_XDR &&  f77xdr[*fid]!=NULL && pos!=NULL) {
    *pos=(int)xdrfile_getpos(f77xdr[*fid]);
    *ret=0;
  } else
    *ret=1;
}
...
void
F77_FUNC(xdrsetpos,XDRSETPOS)(int *fid, int *pos, int *ret)
{
  if(*fid>=0 && *fid<MAX_FORTRAN_XDR &&  f77xdr[*fid]!=NULL && pos!=NULL) {
    xdrfile_setpos(f77xdr[*fid],*pos);
    *ret=0;
  } else
    *ret=1;
}
...
void
F77_FUNC(xdrrint,XDRRINT)(int *fid, int *ip, int *ndata, int *ret)
{
  *ret = xdrfile_read_int(ip,*ndata,f77xdr[*fid]);
}

void
F77_FUNC(xdrwint,XDRWINT)(int *fid, int *ip, int *ndata, int *ret)
{
  *ret = xdrfile_write_int(ip,*ndata,f77xdr[*fid]);
}

void
F77_FUNC(xdrruint,XDRRUINT)(int *fid, unsigned int *ip, int *ndata, int *ret)
{
  *ret = xdrfile_read_u_int(ip,*ndata,f77xdr[*fid]);
}

void
F77_FUNC(xdrwuint,XDRWUINT)(int *fid, unsigned int *ip, int *ndata, int *ret)
{
  *ret = xdrfile_write_u_int(ip,*ndata,f77xdr[*fid]);
}
...
{
  *ret = xdrfile_read_u_char(ip,*ndata,f77xdr[*fid]);
}

void
F77_FUNC(xdrwuchar,XDRWUCHAR)(int *fid, unsigned char *ip, int *ndata, int *ret)
{
  *ret = xdrfile_write_u_char(ip,*ndata,f77xdr[*fid]);
}
...
{
  *ret = xdrfile_read_u_short(ip,*ndata,f77xdr[*fid]);
}

void
F77_FUNC(xdrwushort,XDRWUSHORT)(int *fid, unsigned short *ip, int *ndata, int *ret)
{
  *ret = xdrfile_write_u_short(ip,*ndata,f77xdr[*fid]);
}
...
void
F77_FUNC(xdrrdouble,XDRRDOUBLE)(int *fid, double *dp, int *ndata, int *ret)
{
  *ret = xdrfile_read_double(dp,*ndata,f77xdr[*fid]);
}

void
F77_FUNC(xdrwdouble,XDRWDOUBLE)(int *fid, double *dp, int *ndata, int *ret)
{
  *ret = xdrfile_write_double(dp,*ndata,f77xdr[*fid]);
}
...
void
F77_FUNC(xdrropaque,XDRROPAQUE)(int *fid, char *cp, int *cnt, int *ret)
{
  *ret = xdrfile_read_opaque(cp,*cnt,f77xdr[*fid]);
}

void
F77_FUNC(xdrwopaque,XDRWOPAQUE)(int *fid, char *cp, int *cnt, int *ret)
{
  *ret = xdrfile_write_opaque(cp,*cnt,f77xdr[*fid]);
}


/* Read single-precision compressed 3d coordinates */
void
F77_FUNC(xdrrccs,XDRRCCS)(int *fid, float *data, int *ncoord, 
			  float *precision, int *ret)
{
  *ret = xdrfile_read_compr_coord_float(data,ncoord,precision,f77xdr[*fid]);
}


/* Write single-precision compressed 3d coordinates */
void
F77_FUNC(xdrwccs,XDRWCCS)(int *fid, float *data, int *ncoord, 
			  float *precision, int *ret)
{
*ret = xdrfile_write_compr_coord_float(data,*ncoord,*precision,f77xdr[*fid]);
}


/* Read compressed 3d coordinates into double precision data */
void
F77_FUNC(xdrrccd,XDRRCCD)(int *fid, double *data, int *ncoord, 
			  double *precision, int *ret)
{
  *ret = xdrfile_read_compr_coord_double(data,ncoord,precision,f77xdr[*fid]);
}

/* Write compressed 3d coordinates from double precision data */
void
F77_FUNC(xdrwccd,XDRWCCD)(int *fid, double *data, int *ncoord, 
			  double *precision, int *ret)
{
*ret = xdrfile_write_compr_coord_double(data,*ncoord,*precision,f77xdr[*fid]);
}
...
xdr_u_int (XDR *xdrs, unsigned int *up)
{
  ...
  switch (xdrs->x_op)
    {
    case XDR_ENCODE:
      ui32 = (xdr_uint32_t) * up;
      return xdr_putlong (xdrs, (xdr_int32_t *)&ui32);

    case XDR_DECODE:
      if (!xdr_getlong (xdrs, (xdr_int32_t *)&ui32))
	{
	  return 0;
	}
      *up = (xdr_uint32_t) ui32;
    case XDR_FREE:
      return 1;
    }
  return 0;
     ...
     float *fp;
{
	...
	case XDR_ENCODE:
		if (sizeof(float) == sizeof(xdr_int32_t))
			return (xdr_putlong(xdrs, (xdr_int32_t *)fp));
		else if (sizeof(float) == sizeof(int)) {
			xdr_int32_t tmp = *(int *)fp;
	...
	case XDR_DECODE:
		if (sizeof(float) == sizeof(xdr_int32_t))
			return (xdr_getlong(xdrs, (xdr_int32_t *)fp));
		else if (sizeof(float) == sizeof(int)) {
			xdr_int32_t tmp;
     ...
     double *dp;
{
   ...
   * to calculate it every time. Still, this should be negligible
   * compared to the time it takes to do the actual file IO...
   */
  ...
  if(LSW<0) {
    ...
    if(ix==0xdd || ix==0x3f)
      ...
      fprintf(stderr,"Cannot detect floating-point word order - do you have a non-IEEE system?\n"
	      "Use the system XDR libraries or modify the code in xdr_double().\n");
      abort();
    }
  }  
  ...
  case XDR_ENCODE:
    if (2*sizeof(xdr_int32_t) == sizeof(double)) {
      xdr_int32_t *lp = (xdr_int32_t *)dp;
...
static int xdrstdio_getlong (XDR *, xdr_int32_t *);
static int xdrstdio_putlong (XDR *, xdr_int32_t *);
...
static int
xdrstdio_getlong (XDR *xdrs, xdr_int32_t *lp)
{
  xdr_int32_t mycopy;
...
static int
xdrstdio_putlong (XDR *xdrs, xdr_int32_t *lp)
{
  xdr_int32_t mycopy = xdr_htonl (*lp);
  lp = &mycopy;
...
{
  return fseeko ((FILE *) xdrs->x_private, pos, 0) < 0 ? 0 : 1;
}
